...
def _build_mime_lite_client_work(
  ...
  @computations.tf_computation(init_fn.type_signature.result.member[0],
                               weights_type, data_type)
  def client_update_fn(optimizer_state, initial_weights, data):
    model = model_fn()
    dataset_reduce_fn = dataset_reduce.build_dataset_reduce_fn(
        use_experimental_simulation_loop)

    @tf.function
    def client_update(optimizer_state, initial_weights, data):
      ...
      def train_reduce_fn(state, batch):
        ...
        _, updated_weights = optimizer.next(optimizer_state,
      ...
      return client_works.ClientResult(
          ...
          update_weight=client_weight), model_output, stat_output, full_gradient

    return client_update(optimizer_state, initial_weights, data)
  ...
  def next_fn(state, weights, client_data):
    ...
    client_result, model_outputs, state_output, full_gradient = (
    ...
    stat_metrics = intrinsics.federated_sum(state_output)
...
def build_weighted_mime_lite(
  ...
  """Builds a learning process that performs Mile Lite.

  This function creates a `tff.learning.templates.LearningProcess` that performs
  Mile Lite algorithm on client models. The iterative process has the following
